 ...
 * layout pass.
 *
 * **contract (flat headers only):** every `HeaderCell` has
...
export interface HeaderCell {
   ...
   * always emits `0`. Group headers at `depth > 0` arrive
   * in Phase ~9.
   */
  readonly depth: number;

  /**
   * Horizontal span (colspan). always emits `1`. Group
   ...
   * hover / etc.). Empty array at; populated by feature
