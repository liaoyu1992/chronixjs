...
exports. Independent of any original implementation.
...
discriminator union is the wrong shape — it leads to hit-test bugs that
take multiple patches to fix.
...
Five IR-level fields whose existence prevents the biggest categories
of gantt rendering bugs from being possible in chronix:
...
4. **`requireInitialHit: boolean` on `PointerCaptureSession`.** A named,
   documented option instead of a magic boolean. The semantic name explains
   _why_ you'd want this (separate-overlay subject), not just _what_ it does.
...
4. **distinctiveness over brevity.** A slightly longer name that clearly
   conveys its purpose is preferred over a short ambiguous name.
