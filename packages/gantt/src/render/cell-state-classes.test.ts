...
describe('getDayClassNames', () => {
  ...
  it('NEVER emits cx-gantt-day-other (architectural rejection — chronix-timeline vs original spec calendar-grid model)', () => {
    const today = startOfDay(new Date('2026-05-13T00:00:00'));
    for (const date of [
...
describe('getSlotClassNames', () => {
  ...
  it('NEVER emits cx-gantt-slot-other (matches original spec getSlotClassNames intentional omission)', () => {
    const today = startOfDay(new Date('2026-05-13T00:00:00'));
    for (const date of [
      new Date('2026-05-01T00:00:00'),
      new Date('2026-05-13T12:00:00'),
