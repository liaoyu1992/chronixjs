...
export interface AxisRangePlanInput {
   ...
   * (analog of the original spec's per-segment lane slicer) is parked
...
export interface RowSwimlaneLayoutInput {
   ...
   * the CSS row-divider border (the original `<tr>` row-bottom
...
export interface BarStackHeightPassInput {
  ...
  /** Vertical gap between stacked bars in pixels. Default 5 (matches the original effective spacing). */
  readonly barStackSpacing?: number;
  /** Top padding above the first stacked bar within the row. Default 4 (yields symmetric 4+4 padding with `lastBarBottomPadding` for a single-bar row). */
...
export interface BarPlacementPassInput {
   ...
   * Default 4 (matches the height-pass default `firstBarTopPadding` so
   * single-bar rows render with symmetric 4+4 padding around the bar).
   * When `barHeight` is omitted, this is also the bottom padding (so
   * `bar.height = strip.height - 2 × barVerticalPadding`).
   */
   ...
   *
   * The original spec uses an `eventMinHeight` config (default 30) here
   ...
   * reference's effective `eventSpacing` value).
   *
...
export interface LinkRouterInput {
   ...
   * Default 20 — matches the original spec's empirical value.
   */
