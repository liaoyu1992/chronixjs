...
describe('defaultPointerCaptureSession — BarDrag recording-replay parity', () => {
  it("matches the 'event-drag' recording: +60px pointer drag → +1h time shift on event-7", () => {
    ...
    // The recorded bar shifted by exactly the pointer-delta (1:1 drag).
    ...
    // original spec's observed bar pixel shift. Closes the recording-replay loop.
...
describe('defaultPointerCaptureSession — BarResize recording-replay parity', () => {
  ...
  it("matches 'event-resize-left' recording: chronix start-edge math is unit-consistent with the recorded pointer drag", () => {
    ...
    // Direction parity: the recorded bar's start-edge bbox moved
    // in the same direction as chronix's predicted start-edge time delta.
    // Magnitude is NOT asserted because the original spec applies a
