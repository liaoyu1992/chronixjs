...
export interface ChronixTheme {
   ...
   * (matches original spec behavior where one color drives both).
   */
   ...
   * `'rgba(255, 220, 40, .15)'` matches the original spec's
  ...
  /**
   * Font weight for auto-rendered bar title text. Accepts either a
   * numeric weight (400 = normal, 600 = semibold, 700 = bold) or a
   * CSS keyword (`'normal'`, `'bold'`, etc.) — both render through
   * the SVG `font-weight` attribute identically. Default 400 —
   ...
   * Default `'#ddd'` matches the original `--gantt-border-color`
   * fallback.
   */
   ...
   * matches the original week-emphasis fallback. Unlike the
   * original spec (where one CSS var with two fallbacks means
   ...
   * `'rgba(0,0,0,0.3)'` matches the original hard-coded
   ...
   * matches the original `strokeWidth="2"`. The border is
   ...
   * hit-test. Default `8` matches the original spec's
   ...
   * `-end`). Default `8` matches the original spec's
   * `--gantt-event-resizer-dot-total-width` default. The dot's
   * border-width is hard-coded to 1 px for v0 — the original spec
...
export const defaultChronixTheme: ChronixTheme = {
  ...
  // without the original "one var, two fallbacks" quirk.
  gridLineColor: '#ddd',
