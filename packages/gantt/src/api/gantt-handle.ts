...
import type { HitTestFromClientResult } from './hit-test-from-client.js';
...
export interface GanttHandle {
   ...
   *
   * **Use case**: external drag-and-drop. Wire a consumer's external
   * draggable element via HTML5 DnD / react-dnd / native pointer
   * events; on drag-end call `hitTestFromClient(e.clientX, e.clientY)`
   * to compute the drop's (time, rowId) for new-bar creation. chronix
   * intentionally does not own the drag lifecycle — consumers have
   * their own drag systems and only need this hit-test primitive.
   *
   * Pure read — no emit, no mutation. Safe to call mid-render.
   *
   * Returns `null` when:
   * - the client point is left of / above the chart body
   * - no swimlane strip owns the y (inter-strip gap or below the
   *   last strip)
   * - the axis has no ticks (e.g. before first mount paints)
   */
  hitTestFromClient(clientX: number, clientY: number): HitTestFromClientResult | null;
}
