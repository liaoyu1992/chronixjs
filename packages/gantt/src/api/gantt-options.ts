 ...
 * today.
 *
 * Defaults match the original spec: red (`#ff6b6b`), 2 px wide,
 ...
 * Live "now" updates + sub-second indicator markers are out of scope.
 */
export interface TodayLineOption {
  /**
   * Stroke color for the body + header `<line>`. Also used as the
   * tooltip background (matching original spec behavior where one
   ...
   * - `'dotted'` → `'2 3'`
   *
   * Default `'dashed'`. Original `'double'` and `'dashed-dot'`
 ...
 * the line.
 *
 * Defaults match the original spec: `rgba(255, 220, 40, .15)`
...
export interface TodayCellBgOption {
   ...
   * `rgba(255, 220, 40, .15)` matches the original spec. When
