 ...
 * chronix-react demo.**
 *
 * Mirror of `examples/gantt-vue3/src/sample-callbacks.ts`. The
 * predicates depend only on `@chronixjs/gantt` public types so the
 * file is framework-agnostic — cloned verbatim except for the relative
 * import path. Extracted from `DemoApp.tsx` so the component file
 * holds plumbing (config / template / event handlers) and this file
 * holds the domain predicates that the demo's toggles + parity-mode
 * wiring reference by name.
 *
 ...
 * `<ChronixGantt eventOverlap={false}>` denies all cross-row
