 ...
 * so the implementation is identical. Each schema field declares its
 * type + default + description; the URL is source of truth (shareable,
 * reload-safe, parity-test friendly).
 *
...
export function useDemoConfig<S extends Readonly<Record<string, ConfigField<any>>>>(
  ...
  for (const key of Object.keys(schema)) {
    ...
    // Vue 2.7's `ref()` has a looser-typed signature than Vue 3's; the
    // per-key `field.parse` is intentionally `any` (the schema is keyed
    // generically). Cast through unknown so eslint's unsafe-assignment
    // gate doesn't fire on this dynamic-keyed assignment.
    const initial: unknown = field.parse(raw);
    refs[key] = ref(initial);
  }
