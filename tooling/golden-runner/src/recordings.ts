...
async function pickAndResizeEdge({
  ...
  // Force the resizer rects always-visible. The original-reference defaults
  ...
  // geometry and resize event handlers stay exactly as original renders
  ...
  // chronix-side code stays free of original class names.
...
export const INTERACTION_RECORDINGS: InteractionRecording[] = [
  ...
  },
  {
    ...
    description:
      ...
      "view) to define a new date range. Auto-dismisses the original's " +
    ...
    viewToggleLabel: '日',
    async perform({ page, chart, snapshot, log }) {
      // Auto-dismiss the original's title prompt. We do NOT accept it with a
