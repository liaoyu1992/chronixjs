/**
 * **The boundary between chronix and the original-spec DOM.**
 *
 * This file is the ONE place chronix-side test code is allowed to name
 * the original spec demo's CSS class names and `data-*` attribute
 * names. Everywhere else in chronix code, refer to these by the named
 * constants exported here.
 *
 * Why: chronix is an rewrite — the original demo is the parity
 * oracle, and the parity tests have to query its rendered DOM. The
 * DOM contract (class names, data-attrs) is original-owned and not
...
/**
 * Phase 23: original left-side scroll pane (sidebar /
...
/**
 * `data-*` attribute names. Use these inside `chart.evaluate`
...
/** Event bar lookup by original's source-event id (e.g. "event-7"). */
export const eventBarBySource = (eventId: string): string => `[data-event-id="${eventId}"]`;

/**
 * Event bar lookup by original's instance id. Distinct from source id:
...
/**
 * Today-line root element in the original demo (Phase 21 parity). The
 * original renders ONE `<div class="gantt-timeline-today-line">` in
 ...
 * original div vs. `x1` attribute on the chronix line.
 */
export const TODAY_LINE = '.gantt-timeline-today-line';

/**
 * Phase 22.2: today-column background tint (original spec's
 ...
 * full one-day slot (vs reference's per-row stack). The cross-demo parity
...
/**
 * CSS override that forces the original's resize handles to display:block
 ...
 * the same pointer→bbox trace the original would emit in a real user
...
/**
 * Header toolbar root in the original demo. Chronix equivalent:
...
/**
 * Per-button selector pattern. The original renders one
...
/**
 * Phase 26: vertical grid lines emitted by the original demo per axis
...
/**
 * Phase 26: horizontal row-bottom line emitted by the original demo's
 ...
 * original demo's `TimelineEvent.tsx` when a bar's calendar range
...
/**
 * Phase 28.2: auto-rendered bar title text emitted by the original
 * demo's `TimelineEvent.tsx:543-656`. One `<text class="gantt-event-text">`
 * per event wide enough to show its title (the original spec's
...
/**
 * Phase 28.1: selection-border SVG rect emitted by the original demo's
...
/**
 * Phase 28.1: visible resize-dot handles emitted by the original demo
 ...
 * parity assertion compares that to reference's visible (selected-
 * filtered) dot count.
 */
...
/**
 * Class on the original demo's `<g class="gantt-event">` when the bar
