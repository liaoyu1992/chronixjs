 ...
 *
 * `'cross'`: pixel-diff chronix screenshot vs reference baseline. Requires
 * `urlQuery` to be representable on BOTH demos (original demo doesn't yet
 * expose the URL-config layer, so cross scenarios are limited to
 * combinations where reference's default rendering matches chronix's
 * parity-mode rendering of the same flags).
 *
 * `'vrt'`: pixel-diff chronix screenshot vs chronix baseline. Used
 * when reference has no analog for the chronix toggle (`themedBars`,
...
export interface CrossDemoScenario {
   ...
   * skips the toggle click for week (original demo's default may also
...
export const CROSS_DEMO_SCENARIOS: readonly CrossDemoScenario[] = [
  // ─── Group 1: 12 cross-demo (reference baseline + chronix verify) ────
  ...
  },
  {
    ...
    kind: 'cross',
    description:
      'Phase 20 callback-driven per-bar color, parity palette (callback wired in parity mode so reference default matches).',
...

  {
    ...
    kind: 'vrt',
    description:
      'Phase 20 themedBars prop (chronix-only API; reference has no equivalent). Overrides bar fill + border via component props.',
 ...
 * 5 chronix VRT + 6 reference parity goldens at the goldens root.
 */
export const CROSS_DEMO_BASELINE_DIR = 'cross-demo-baselines';

/** Resolves to e.g. `cross-demo-baselines/reference/cross-week-default.png`. */
export function crossDemoBaselineRelPath(scenario: CrossDemoScenario): string {
  const subdir = scenario.kind === 'cross' ? 'reference' : 'chronix';
