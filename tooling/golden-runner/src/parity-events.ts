 ...
 *
 * The data mirrors the original demo's `generateTestEvents()` at
...
/**
 * The 32 resource ids in the original demo's `RESOURCES[]` array order
 * (NOT the rendered order, which differs because reference groups by
 * `baseName` for the left panel display).
 *
 * Chronix v0 parity mode emits rows in this order, accepting that
 * the rendered Y position will NOT match reference's because chronix
...
export interface ParityEvent {
  ...
  /** 0..100 progress percentage; undefined when the original spec event has no progress field. */
  readonly progressValue?: number;
  /**
   * Phase 28.2: bar title text. Mirrors the original spec demo's
   * per-event `title` field so cross-demo content parity (Set of
   * truncated text contents) holds. Always present — every event
   * in the original fixture carries a title.
   */
 ...
 * original demo's event-generator output; the
 * (id, resourceId, startMs, endMs) fields are the cross-demo
 * identity tuple. Progress values mirror the original fixture's
...
export function buildParityEvents(todayMs: number): readonly ParityEvent[] {
  ...
  // `PARITY_LINKS`. Values mirror the original ACTUALLY-
  // RENDERED bar fill (read from `.gantt-event-background` rects on
  // a running original spec demo), not the per-event
  // `backgroundColor` field from the original demo source: the
...
/**
 * Phase 28.3.1: curated 8-edge subset of the original spec demo's
