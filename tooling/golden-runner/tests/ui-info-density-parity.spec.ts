...
/**
 * chronix-ui 3-adapter Ellipsis + Thing + Log parity spec —
 * Phase 23 (2026-06-03). Same testid → identical DOM fingerprint
 * (tag + sorted class set + role) across vue3 / vue2 / react.
 *
 * Ellipsis root is `<span>` (23-fr2); Thing root is `<div>`; Log
 * root is `<div>` (with inner `<ol>` lines container).
 */

const ELLIPSIS_TESTIDS = ['ellipsis-single', 'ellipsis-multi-line', 'ellipsis-no-tooltip'] as const;

const THING_TESTIDS = ['thing-basic', 'thing-with-avatar', 'thing-full', 'thing-indented'] as const;
...
makeParityBlock('chronix-ui / Ellipsis 3-adapter parity', ELLIPSIS_TESTIDS, 'ellipsis');
makeParityBlock('chronix-ui / Thing 3-adapter parity', THING_TESTIDS, 'thing');
makeParityBlock('chronix-ui / Log 3-adapter parity', LOG_TESTIDS, 'log');
