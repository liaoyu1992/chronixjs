...
/**
 * chronix-ui Phase 31 Tier C Select family parity — 2026-06-05.
 * 5 testids fingerprinted across vue3 / vue2 / react +
 * 4 stylesheet injection counts.
 */

const PHASE31_TESTIDS = [
  'phase31-select',
  'phase31-select-multi',
  'phase31-tree-select',
  'phase31-cascader',
  'phase31-mention',
] as const;
...
test.describe('chronix-ui / Phase 31 Tier C Select family 3-adapter parity', () => {
  for (const testid of PHASE31_TESTIDS) {
  ...
  test('every demo injects select / tree-select / cascader / mention stylesheets', async ({
    browser,
  }) => {
    const ctx = await browser.newContext();
    ...
    try {
      await Promise.all([
        pages[0].goto(CHRONIX_UI_VUE3_DEMO_URL),
        pages[1].goto(CHRONIX_UI_VUE2_DEMO_URL),
        pages[2].goto(CHRONIX_UI_REACT_DEMO_URL),
      ]);
      for (const componentName of ['select', 'tree-select', 'cascader', 'mention']) {
        const counts = await Promise.all(
          pages.map((p) => p.locator(`style[data-chronix-ui="${componentName}"]`).count()),
        );
        expect(counts, `style[data-chronix-ui="${componentName}"]`).toEqual([1, 1, 1]);
      }
    } finally {
      await ctx.close();
    }
