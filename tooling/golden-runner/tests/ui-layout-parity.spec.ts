...
/**
 * chronix-ui 3-adapter Space + Flex + Grid parity spec — Phase 17
 * (2026-06-02). Same testid → identical DOM fingerprint (tag + sorted
 * class set + role) across vue3 / vue2 / react.
 *
 * Reuses the `makeParityBlock` pattern from
 * ui-info-containers-parity.spec.ts / ui-loading-states-parity.spec.ts.
 */
...
const GRID_TESTIDS = ['grid-3col', 'grid-12col', 'grid-custom', 'grid-inline'] as const;
...
makeParityBlock('chronix-ui / Space 3-adapter parity', SPACE_TESTIDS, 'space');
makeParityBlock('chronix-ui / Flex 3-adapter parity', FLEX_TESTIDS, 'flex');
makeParityBlock('chronix-ui / Grid 3-adapter parity', GRID_TESTIDS, 'grid');
