...
import { CHRONIX_VUE2_DEMO_URL } from '../src/parity-helpers.js';
...
test.describe('chronix gantt visual goldens (chronix-vue2 demo at 8703)', () => {
  for (const scenario of VISUAL_SCENARIOS) {
    ...
    test(scenario.id, async ({ page }) => {
      ...
      await page.goto(CHRONIX_VUE2_DEMO_URL);
      ...
      await expect(wrapper).toHaveScreenshot(`chronix-vue2/${scenario.id}.png`);
    });
  }
});
