...
import { CHRONIX_UI_REACT_DEMO_URL } from '../src/config.js';

/**
 * chronix-ui react Phase 12 Button smoke spec — Phase 12 (2026-06-02).
 *
 * Drives the dev server at port 8733 (start with
 * `pnpm --filter @chronixjs/example-ui-react dev` before running this
 * spec). Verbatim port of `ui-vue3-smoke.spec.ts` with `CHRONIX_UI_VUE3_DEMO_URL`
 * → `CHRONIX_UI_REACT_DEMO_URL`; same 8 assertions across variant /
...
test.describe('chronix-ui react / Phase 12 Button smoke', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(CHRONIX_UI_REACT_DEMO_URL);
