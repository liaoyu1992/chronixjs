...
test.describe('crossDemoBaselineRelPath', () => {
  test('cross scenarios resolve to cross-demo-baselines/reference/<id>.png', () => {
    const cross = CROSS_DEMO_SCENARIOS.find((s) => s.kind === 'cross');
    if (!cross) throw new Error('expected at least one cross scenario');
    expect(crossDemoBaselineRelPath(cross)).toBe(
      `${CROSS_DEMO_BASELINE_DIR}/reference/${cross.id}.png`,
    );
  });
