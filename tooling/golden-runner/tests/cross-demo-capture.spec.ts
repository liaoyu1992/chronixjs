 ...
 * - `kind: 'cross'` → captures reference side → writes to
 *   `goldens/cross-demo-baselines/reference/<id>.png`
 ...
 * - reference original changes intentionally (cross baselines)
...
test.describe('Phase 20.7 — cross-demo baseline capture (one-time)', () => {
  ...
  for (const scenario of CROSS_DEMO_SCENARIOS) {
    const captureSource: 'reference' | 'chronix' =
      scenario.kind === 'cross' ? 'reference' : 'chronix';
