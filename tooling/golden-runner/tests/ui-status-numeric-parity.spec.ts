...
/**
 * chronix-ui 3-adapter Result + Statistic + Countdown parity spec —
 * Phase 18 (2026-06-02). Same testid → identical DOM fingerprint
 * (tag + sorted class set + role) across vue3 / vue2 / react.
 *
 * Countdown testids in the demo are mounted with `active=false` so
 * the rendered text is deterministic at fingerprint time (no
 * tick-driven divergence between adapters).
 */
...
const COUNTDOWN_TESTIDS = ['countdown-basic', 'countdown-precise', 'countdown-with-label'] as const;
...
makeParityBlock('chronix-ui / Result 3-adapter parity', RESULT_TESTIDS, 'result');
makeParityBlock('chronix-ui / Statistic 3-adapter parity', STATISTIC_TESTIDS, 'statistic');
makeParityBlock('chronix-ui / Countdown 3-adapter parity', COUNTDOWN_TESTIDS, 'countdown');
