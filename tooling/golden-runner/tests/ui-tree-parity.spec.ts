...
/**
 * chronix-ui Phase 30 Tier C Tree parity — 2026-06-04.
 * 2 testids (phase30-tree + phase30-tree-virtual) fingerprinted
 * across vue3 / vue2 / react.
 */

const PHASE30_TESTIDS = ['phase30-tree', 'phase30-tree-virtual'] as const;
...
test.describe('chronix-ui / Phase 30 Tier C Tree 3-adapter parity', () => {
  for (const testid of PHASE30_TESTIDS) {
  ...
  test('every demo injects exactly one tree stylesheet', async ({ browser }) => {
    const ctx = await browser.newContext();
    const [vue3Page, vue2Page, reactPage] = await Promise.all([
      ctx.newPage(),
      ctx.newPage(),
    ...
    try {
      ...
      const counts = await Promise.all(
        [vue3Page, vue2Page, reactPage].map((p) =>
          p.locator('style[data-chronix-ui="tree"]').count(),
        ),
      );
      expect(counts, 'style[data-chronix-ui="tree"]').toEqual([1, 1, 1]);
    } finally {
      await ctx.close();
    }
  });
