 ...
 * 8736×1315 px vs k-ui's wrapper at 8737×1318 px (1-px narrower, 3-px
 ...
 * - k-ui's screenshot target is `.gantt-timeline-body-wrapper` (a `div`
  ...
  for (const scenario of CROSS_DEMO_SCENARIOS) {
    ...
    test(`${scenario.kind} :: ${scenario.id}`, async ({ browser }) => {
      if (PENDING_CROSS_DIMENSION_FIX.has(scenario.id)) {
        test.fail(
          true,
          'Phase 20.7.1: cross-demo body-region dimension mismatch (1-px width / 3-px height between chronix svg.cx-gantt-body and k-ui timeline-body-wrapper). Root-cause + fix pending — see test file header comment.',
        );
      }
