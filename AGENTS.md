...
- `.qoder/` is a Qoder CLI self-evolution system with hooks/skills/memory. The AI gateway config lives at `.qoder/config/ai-gateway.env` (gitignored — contains real API keys). A template with placeholder values is committed at `.qoder/config/ai-gateway.env.example`; copy it to `ai-gateway.env` and fill in your Zhipu/GLM credentials to enable the auto-analyze and memory-extraction hooks.
