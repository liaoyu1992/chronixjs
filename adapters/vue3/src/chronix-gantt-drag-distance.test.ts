...
// gives slotWidth=60 (24 hourly slots) so bar geometry is
// predictable: a 4-hour bar at hours 8-12 spans content x=480..720.
...
describe('<ChronixGantt> drag-distance gate — Phase 25', () => {
  ...
  it('sub-threshold pointer-wiggle (3 px) on bar body fires `bar-click` not `bar-drop`', async () => {
    ...
    // Press at (600, 20), wiggle to (603, 20) = Pythagorean 3 < 5,
    // release. Should NOT emit bar-drop; SHOULD emit bar-click.
    await svg.trigger('pointerdown', { clientX: 600, clientY: 20, button: 0, pointerId: 1 });
    await svg.trigger('pointermove', { clientX: 603, clientY: 20, pointerId: 1 });
    await svg.trigger('pointerup', { clientX: 603, clientY: 20, pointerId: 1 });
  ...
  it('above-threshold pointer-wiggle (10 px) on bar body fires `bar-drop` not `bar-click`', async () => {
    ...
    await svg.trigger('pointerdown', { clientX: 600, clientY: 20, button: 0, pointerId: 1 });
    await svg.trigger('pointermove', { clientX: 610, clientY: 20, pointerId: 1 });
    await svg.trigger('pointerup', { clientX: 610, clientY: 20, pointerId: 1 });
  ...
  it('pointerMinDistance=0 restores pre-Phase-25 behavior — any non-zero delta commits a drag', async () => {
    ...
    await svg.trigger('pointerdown', { clientX: 600, clientY: 20, button: 0, pointerId: 1 });
    await svg.trigger('pointermove', { clientX: 601, clientY: 20, pointerId: 1 });
    await svg.trigger('pointerup', { clientX: 601, clientY: 20, pointerId: 1 });
  ...
  it('sub-threshold resize gesture on bar edge does NOT commit a resize', async () => {
    ...
    // abort path which also dropped them).
    await svg.trigger('pointerdown', { clientX: 716, clientY: 20, button: 0, pointerId: 1 });
    await svg.trigger('pointermove', { clientX: 718, clientY: 20, pointerId: 1 });
    await svg.trigger('pointerup', { clientX: 718, clientY: 20, pointerId: 1 });
  ...
  it('sub-threshold range-select gesture on empty row does NOT commit a select', async () => {
    ...
    await svg.trigger('pointerdown', { clientX: 200, clientY: 2, button: 0, pointerId: 1 });
    await svg.trigger('pointermove', { clientX: 203, clientY: 2, pointerId: 1 });
    await svg.trigger('pointerup', { clientX: 203, clientY: 2, pointerId: 1 });
