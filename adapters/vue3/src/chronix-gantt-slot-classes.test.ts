...
describe('<ChronixGantt> body slot rect emission — Phase 29', () => {
  ...
  it('slot rect width matches axis.slotWidth and height matches body content height', () => {
    ...
    // = max(60, 52) = 60. Body content height = 1 strip × 38 default
    // row height = 38, plus 0 row spacing.
    ...
    // height = single row height (38) since rows has 1 entry.
    expect(rects[0]!.attributes('height')).toBe('38');
  });
