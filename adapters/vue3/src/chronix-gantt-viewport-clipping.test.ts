 ...
 * `useChartScrollState` updates both refs. jsdom doesn't auto-fire
 * scroll events on `el.scrollLeft = X` assignment, so the test
 * dispatches the event manually; `clientWidth` is a read-only DOM
 * proxy in jsdom, so it's `Object.defineProperty`-overridden so the
...
describe('<ChronixGantt> viewport-clipping continuation triangles — Phase 27.1', () => {
  ...
  it('axis-clipped bar emits triangle anchored to the bar edge with data-viewport-clipped="false"', async () => {
    ...
    const wrapper = mount(ChronixGantt, {
      props: { bars: [bar('crosses-end', 10, 30)], rows, axisInput },
    });
  ...
  it('viewport-clipped bar (axis-inside, viewport-spanning) emits triangles at viewport edges with data-viewport-clipped="true"', async () => {
    ...
    const wrapper = mount(ChronixGantt, {
      props: { bars: [bar('inside-axis', 1, 16)], rows, axisInput },
    });
  ...
  it('bar that is BOTH axis-clipped AND viewport-clipped on same side: viewport-locked apex wins (precedence)', async () => {
    ...
    // renderX + 1 = -239).
    const wrapper = mount(ChronixGantt, {
      props: { bars: [bar('both-clipped-left', -4, 6)], rows, axisInput },
    });
  ...
  it('pre-mount frame (clientWidth = 0) does NOT emit phantom viewport-clipped triangles', async () => {
    ...
    const wrapper = mount(ChronixGantt, {
      props: { bars: [bar('fully-inside', 1, 3)], rows, axisInput },
    });
