...
describe('ChronixWatermark — root rendering', () => {
  it('renders a <div> with the base class', () => {
    const wrapper = mount(ChronixWatermark);
...
describe('ChronixWatermark — inline style', () => {
  ...
  it('changes background-image when content prop changes', async () => {
    const wrapper = mount(ChronixWatermark, { props: { content: 'A' } });
