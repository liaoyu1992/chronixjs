...
describe('ChronixTimePicker (Vue 3)', () => {
  it('renders root element with correct class', () => {
    const wrapper = mount(ChronixTimePicker);
  ...
  it('shows placeholder when no value', () => {
    const wrapper = mount(ChronixTimePicker, { props: { placeholder: 'Pick time' } });
    expect(wrapper.text()).toContain('Pick time');
  });

  it('shows formatted value when value is set', () => {
    const date = new Date(2026, 5, 15, 14, 30, 0);
    const wrapper = mount(ChronixTimePicker, { props: { value: date, format: 'HH:mm:ss' } });
    expect(wrapper.text()).toContain('14:30:00');
  });

  it('adds disabled class when disabled', () => {
    const wrapper = mount(ChronixTimePicker, { props: { disabled: true } });
