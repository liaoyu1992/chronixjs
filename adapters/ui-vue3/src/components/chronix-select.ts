 ...
 * - `update:value(value: string | string[])` — selected value(s).
 */
    ...
    return () => {
      ...
      if (props.loading) {
        ...
        const optionNodes = flatEntries.value.map((entry: FlatOptionEntry) => {
          ...
          const leafValue = !isGroup ? (opt as { value: string }).value : undefined;
          ...
          return h(
            'div',
            {
              ...
              'data-testid': isGroup ? undefined : `select-option-${opt.key}`,
              onMousedown: (e: MouseEvent) => {
                e.preventDefault();
                if (!isGroup) selectOption(opt.key);
              },
              onMouseenter: () => {
                if (!isGroup) focusedKey.value = opt.key;
              },
            },
            opt.label,
          );
