...
describe('<ChronixGantt> progress overlay — text label', () => {
  it('renders the default "{value}%" label centered on the bar', () => {
    ...
    // Bar at x ∈ [480, 720] → center x = 600. y-mid = 19 (+4 for baseline)
    // (Phase 43: bar.y = 4 instead of 8 due to barVerticalPadding default shift).
    expect(Number(label.attributes('x'))).toBe(600);
    expect(Number(label.attributes('y'))).toBe(23);
  });
