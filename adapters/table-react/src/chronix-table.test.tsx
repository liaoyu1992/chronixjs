...
describe('<ChronixTable> (react).1 virtualRowsPass + scrollport (vue3)', () => {
  ...
  it('header mirrors the body scrollbar gutter; footer is sticky inside body', () => {
    ...
    const headerEl = container.querySelector('.cx-table-header');
    const headerStyle = headerEl?.getAttribute('style') ?? '';
    expect(headerStyle).toMatch(/overflow-y:\s*scroll/i);
...
describe('<ChronixTable> — (pinned columns)', () => {
  ...
  it('pinned filter-row cells also get sticky positioning + zone modifier classes', () => {
    ...
    const { container } = render(<ChronixTable columns={pinnedCols} rows={rows} />);
    // Header cells inherit pinned styling.
    const leftHeader = container.querySelector<HTMLElement>(
      '.cx-table-header-cell[data-col-id="id"]',
    )!;
    const rightHeader = container.querySelector<HTMLElement>(
      '.cx-table-header-cell[data-col-id="note"]',
    )!;
