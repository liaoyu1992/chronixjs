 ...
 * - `attrs: {}` + `on: {}` nesting in h() VNodeData
 * - Tests use `as unknown as VueConstructor` mount cast
 */
...
const FORM_INJECTION_KEY = Symbol('ChronixFormContext');
...
export const ChronixForm = defineComponent({
  ...
  setup(props, setupCtx) {
    ...
    setupCtx.expose({ validate, restoreValidation });
    ...
    return () =>
      h(
        'form',
        {
          ...
          ...(labelWidthStyle.value !== undefined ? { style: labelWidthStyle.value } : {}),
          attrs: { 'data-testid': 'form-root', ...setupCtx.attrs },
          on: {
            submit: (e: Event) => {
        ...
        setupCtx.slots['default'] ? setupCtx.slots['default']() : [],
      );
  },
});
...
export const ChronixFormItem = defineComponent({
  ...
  setup(props, ctx) {
    ...
    async function validate(trigger?: string): Promise<readonly FieldError[]> {
      ...
      if (trigger) {
        ...
        const filtered = arr.filter((r) => {
          const tr = (r as TriggerableRule).trigger;
          return !tr || tr === trigger || (Array.isArray(tr) && tr.includes(trigger));
        });
    ...
    ctx.expose({ validate, restoreValidation });
    ...
    return () => {
      ...
      if (hasLabel) {
        ...
        if (resolvedShowRequireMark()) {
          labelChildren.push(h('span', { class: 'cx-ui-form-item-label__asterisk' }, '*'));
        }
        labelChildren.push(h('span', { class: 'cx-ui-form-item-label__text' }, props.label));
        children.push(h('label', { class: labelClass, style: labelStyle }, labelChildren));
      }
      ...
      children.push(
        h(
          'div',
          { class: blankClass, attrs: { 'data-testid': 'form-item-blank' } },
          ctx.slots['default'] ? ctx.slots['default']() : [],
        ),
      );

      // Feedback
      if (resolvedShowFeedback() && hasError) {
        children.push(
          h(
            'div',
            { class: feedbackClass },
      ...
      return h(
        'div',
        { class: itemClass, attrs: { ...ctx.attrs, 'data-testid': 'form-item' } },
        children,
      );
    };
  },
