...
export const ChronixTreeSelect = defineComponent({
  ...
  setup(props, { emit, attrs }) {
    ...
    const visibleRows = computed(() =>
      resolveVisibleTreeRows({ items: props.data, expandedKeys: expandedKeySet.value }),
    );
    ...
    function removeTag(value: string): void {
      ...
      emit(
        'update:value',
        normalizedValue.value.filter((v) => v !== value),
      );
    }
        ...
        for (let i = 0; i < normalizedValue.value.length; i++) {
          ...
          triggerChildren.push(
            h('span', { class: resolveTreeSelectTagClassList().join(' '), key: `tag-${val}` }, [
              h('span', undefined, selectedLabels.value[i] ?? val),
              h(
                'span',
                {
          ...
          treeContent.push(
            h(
              'div',
              {
                ...
                class: rowClasses,
                style: indentStyle,
                attrs: { 'data-testid': `tree-select-row-${key}` },
                on: {
                  click: () => {
                    if (!isDisabled) {
        ...
        children.push(
          h(
            'div',
            {
              ...
              attrs: { 'data-testid': 'tree-select-dropdown-popup' },
            },
