...
export const ChronixTree = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function renderRow(entry: (typeof visibleRows.value)[number]): VNode {
      ...
      const rowOn: Record<string, (...args: never[]) => void> = {
      ...
      if (props.draggable && !isDisabled) {
        ...
        rowOn['dragstart'] = (e: DragEvent) => onDragStart(e, key);
        rowOn['dragover'] = (e: DragEvent) => onDragOver(e, key);
        rowOn['dragleave'] = () => onDragLeave();
        rowOn['drop'] = (e: DragEvent) => onDrop(e, key);
        rowOn['dragend'] = () => onDragEnd();
      }

      return h('div', { key, class: rowClass, attrs: rowAttrs, on: rowOn }, children);
    }
    ...
    return () => {
      ...
      if (props.virtual && virtualWindow.value) {
        ...
        rootChildren.push(h('div', { style: { height: `${vw.offsetTopPx}px` } }));
        const visible = visibleRows.value.slice(vw.startIndex, vw.endIndex);
        rootChildren.push(...visible.map(renderRow));
        rootChildren.push(
          h('div', {
            style: {
        ...
        return h(
          'div',
          {
            ...
            attrs: { role: 'tree', tabindex: '0', 'aria-multiselectable': false },
