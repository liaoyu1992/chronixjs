...
const Timeline = ChronixTimeline as unknown as VueConstructor;
...
describe('ChronixTimeline (vue2) — root rendering', () => {
  it('renders a <div> with the base class', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
...
describe('ChronixTimeline (vue2) — items', () => {
  ...
  it('applies color modifier per item', () => {
    ...
    expect(items.at(0).classes()).toContain('cx-ui-timeline__item--color-success');
    expect(items.at(1).classes()).toContain('cx-ui-timeline__item--color-info');
  });

  it('applies lineType modifier per item', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
    const items = wrapper.findAll('.cx-ui-timeline__item');
    expect(items.at(1).classes()).toContain('cx-ui-timeline__item--line-dashed');
  });
});

describe('ChronixTimeline (vue2) — last-item special case', () => {
  it('only the last item carries the --last modifier', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
    const items = wrapper.findAll('.cx-ui-timeline__item');
    expect(items.at(0).classes()).not.toContain('cx-ui-timeline__item--last');
    expect(items.at(2).classes()).toContain('cx-ui-timeline__item--last');
  });

  it('only the last item omits the __line element', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
    const items = wrapper.findAll('.cx-ui-timeline__item');
    expect(items.at(0).find('.cx-ui-timeline__line').exists()).toBe(true);
    expect(items.at(2).find('.cx-ui-timeline__line').exists()).toBe(false);
  });
...
describe('ChronixTimeline (vue2) — title / description / timestamp', () => {
  it('renders __title with item title text', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
    expect(wrapper.findAll('.cx-ui-timeline__title').at(0).text()).toBe('Created');
  });
  ...
  it('renders __timestamp only when timestamp is defined', () => {
    const wrapper = mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
    expect(wrapper.findAll('.cx-ui-timeline__timestamp')).toHaveLength(2);
  });
});

describe('ChronixTimeline (vue2) — CSS injection', () => {
  it('mounting ensures the chronix-timeline stylesheet is in document.head', () => {
    mount(Timeline, { propsData: { items: SAMPLE_ITEMS } });
