        ...
        ? h(
            'div',
            {
              ...
              style: {
                ...
                // scrollTop. overflow-x is scroll (not auto): ALWAYS reserve
                // a horizontal-scrollbar strip matching the chart-pane's
                // bottom scrollbar thickness (same browser default → equal
                // clientHeight), so the 1:1 translateY sync keeps the last
                // row above the chart's scrollbar (with `auto` the sidebar
                // shows no bar — its table is width:100% — the panes'
                // heights diverge and the chart's bottom bar is occluded).
                // A narrowed sidebar that overflows its columns gets a
                // draggable bar; otherwise the thumb fills the track.
                overflowX: 'scroll',
                overflowY: 'auto',
