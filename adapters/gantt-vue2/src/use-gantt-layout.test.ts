...
describe('useGanttLayout (vue2 port)', () => {
  ...
  it('accepts both ref and getter input forms (MaybeRefOrGetter shim)', () => {
    const barsRef = ref<readonly BarSpec[]>([bar('b1', 'r1', 8, 12)]);
    const out1 = useGanttLayout({
      bars: barsRef, // ref form
      rows: () => rowsBase,
      axisInput: () => makeDayAxis(),
    });
    const out2 = useGanttLayout({
      bars: () => [bar('b1', 'r1', 8, 12)], // getter form (equivalent input)
      rows: () => rowsBase,
      axisInput: () => makeDayAxis(),
    });
    // Both forms produce structurally identical placement output.
    expect(out1.placedBars.value[0]?.x).toBe(out2.placedBars.value[0]?.x);
    expect(out1.placedBars.value[0]?.width).toBe(out2.placedBars.value[0]?.width);
  });
