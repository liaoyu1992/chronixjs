...
// Vue 2 test-utils @1.x's `mount` overload accepts `VueConstructor`; the
// Composition-API `defineComponent` return type is structurally similar
// but not assignable, so the cast is the established vue2-adapter test
// pattern (matches `chronix-gantt-dual-scrollport.test.ts`).
...
/**
 * Force a chart-pane scroll + simulate the ResizeObserver callback so
 * `useChartScrollState` updates both refs. happy-dom doesn't auto-fire
 * scroll events on `el.scrollLeft = X` assignment, so the test
 * dispatches the event manually; `clientWidth` is a read-only DOM
 * proxy in happy-dom, so it's `Object.defineProperty`-overridden so the
 * `ResizeObserver` callback (which `useChartScrollState` calls in
 * response to its own observe + the stubbed RO below) reads the
 * test-provided value.
 */
...
describe('<ChronixGantt> viewport-clipping continuation triangles — Phase 31.5.2.1', () => {
  ...
  it('axis-clipped bar emits triangle anchored to the bar edge with data-viewport-clipped="false"', async () => {
    ...
    const wrapper = mount(GanttForTest, {
      propsData: { bars: [bar('crosses-end', 10, 30)], rows, axisInput },
    });
    driveChartScroll(wrapper, 0, 2000, roCallbackHolder.cb);
    await nextTick();

    const rightTri = wrapper.find('polygon.cx-gantt-bar-continuation-right');
    expect(rightTri.exists()).toBe(true);
    expect(rightTri.attributes('data-viewport-clipped')).toBe('false');
    const apexX = Number(rightTri.attributes('points')!.split(' ')[0]!.split(',')[0]);
    expect(apexX).toBe(1799);
  });

  it('viewport-clipped bar (axis-inside, viewport-spanning) emits triangles at viewport edges with data-viewport-clipped="true"', async () => {
    ...
    const wrapper = mount(GanttForTest, {
      propsData: { bars: [bar('inside-axis', 1, 16)], rows, axisInput },
    });
    driveChartScroll(wrapper, 300, 600, roCallbackHolder.cb);
    await nextTick();

    const leftTri = wrapper.find('polygon.cx-gantt-bar-continuation-left');
    const rightTri = wrapper.find('polygon.cx-gantt-bar-continuation-right');
    expect(leftTri.exists()).toBe(true);
    expect(rightTri.exists()).toBe(true);
    expect(leftTri.attributes('data-viewport-clipped')).toBe('true');
    expect(rightTri.attributes('data-viewport-clipped')).toBe('true');
    // Apex locks to viewport edges: left at scrollLeft + 1 = 301; right at
    // scrollLeft + clientWidth - 1 = 899.
    const leftApex = Number(leftTri.attributes('points')!.split(' ')[0]!.split(',')[0]);
    const rightApex = Number(rightTri.attributes('points')!.split(' ')[0]!.split(',')[0]);
    expect(leftApex).toBe(301);
    expect(rightApex).toBe(899);
  });

  it('bar that is BOTH axis-clipped AND viewport-clipped on same side: viewport-locked apex wins (precedence)', async () => {
    ...
    // renderX + 1 = -239).
    const wrapper = mount(GanttForTest, {
      propsData: { bars: [bar('both-clipped-left', -4, 6)], rows, axisInput },
    });
    driveChartScroll(wrapper, 100, 500, roCallbackHolder.cb);
    await nextTick();

    const leftTri = wrapper.find('polygon.cx-gantt-bar-continuation-left');
    expect(leftTri.exists()).toBe(true);
    expect(leftTri.attributes('data-viewport-clipped')).toBe('true');
    const apexX = Number(leftTri.attributes('points')!.split(' ')[0]!.split(',')[0]);
    expect(apexX).toBe(101);
  });

  it('scroll event re-derives triangle state reactively (count changes after scroll)', async () => {
    ...
    // Bar1 [60, 180): both edges < 400 → both viewport-clipped → 2 triangles.
    // Bar2 [300, 420): left edge 300 < 400 → clip-start triangle. Bar3 [600,
    // 720): fully in viewport → 0 triangles.
    const wrapper = mount(GanttForTest, {
      propsData: {
    ...
    driveChartScroll(wrapper, 0, 500, roCallbackHolder.cb);
    await nextTick();
    const countBefore = wrapper.findAll('polygon.cx-gantt-bar-continuation-indicator').length;

    driveChartScroll(wrapper, 400, 500, roCallbackHolder.cb);
    await nextTick();
    const countAfter = wrapper.findAll('polygon.cx-gantt-bar-continuation-indicator').length;
  ...
  it('pre-mount frame (clientWidth = 0) does NOT emit phantom viewport-clipped triangles', async () => {
    // Mount with a fully-contained bar (no axis-clipping). On the first
    ...
    const wrapper = mount(GanttForTest, {
      propsData: { bars: [bar('fully-inside', 1, 3)], rows, axisInput },
    });
    // Do NOT fire the ResizeObserver — clientWidth stays at happy-dom default 0.
    await nextTick();
    const triangles = wrapper.findAll('polygon.cx-gantt-bar-continuation-indicator');
