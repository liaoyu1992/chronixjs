...
describe('<ChronixTable> (vue2)', () => {
  ...
  it('header / filter mirror the body scrollbar gutter; footer is sticky inside body', () => {
    ...
    // header is an external sibling that reserves a matching gutter.
    for (const sel of ['.cx-table-header']) {
  ...
  // ────────────────────────── filterPass + 3 handle methods ──────────────────────────
  // Verbatim ports of vue3 SFC wiring guards (commit `89b1a3e`).
  // Two assertions: setFilter + filter-change emit + getFilter round-trip;
  // setFilter narrows body rows (filter pipeline wired).
  ...
  it('showPagination + 50 rows + initialPageSize=20 → 20 rendered rows + footer present + getTotalPages=3', () => {
  ...
  it('page-number bar renders one button per page when totalPages <= threshold (5 pages, no ellipsis)', () => {
    ...
    const wrapper = mount(TableForTest, {
      propsData: { columns, rows: manyRows, showPagination: true, initialPageSize: 10 },
    });
  ...
  it('ellipsis appears with large totalPages (200 rows / pageSize 10 = 20 pages)', () => {
    ...
    const wrapper = mount(TableForTest, {
      propsData: { columns, rows: lotsOfRows, showPagination: true, initialPageSize: 10 },
    });
...
describe('+ 45.2: server-side refinements (vue2)', () => {
  ...
  it('45.1: showPagination + serverSide mounts without throwing (vue2)', async () => {
    const { source, calls } = makeControlledSource();
    const wrapper = mount(TableForTest, {
      propsData: {
        columns,
...
describe('server-side anticipatory prefetch (vue2)', () => {
  ...
  it('45.5: showPagination=true ignores serverSidePrefetchAheadBlocks prop (vue2)', async () => {
    const { source, calls } = makeControlledSource();
    const wrapper = mount(TableForTest, {
      propsData: {
        columns,
...
describe('tool-panel tablist keyboard nav (vue2)', () => {
  ...
  it('tool-panel: injects synthetic settings column when no actions column exists (vue2)', () => {
    const wrapper = mount(TableForTest, {
