...
export const ChronixTable = defineComponent({
  ...
  setup(props, ctx) {
    ...
    // (2026-05-26 — vue2 port of vue3): header is
    // a SIBLING of body, so the body's `overflowX: auto`
    // scroll does NOT propagate to it. The SFC mirrors
    // `bodyScrollLeft → headerEl.scrollLeft`
    // via the body's `on: { scroll }` handler so the header
    // cells stay column-aligned with body cells during horizontal
    // scroll. Header carries `overflowX: hidden` inline so
    ...
    return () => {
        ...
        return h(
          'div',
          {
            ...
            style: {
              ...
              // over columns that have no parent group — so the group strip is a
              // uniform band (center empty cells used to be transparent, which
              // made grouped vs ungrouped columns look inconsistent).
        ...
        ? (() => {
            ...
            function buildFooterCell(col: ColumnSpec): VNode {
              ...
              const style: Record<string, string> = {
                ...
                // flex-shrink that would misalign aggregate cells.
                boxSizing: 'border-box',
                width: `${width}px`,
            ...
            const selectionPlaceholder: VNode | null = selectionColShow
              ...
              : null;
            // row-drag rail placeholder so the footer's aggregate cells
            // line up with the body's grip column.
                    ...
                    const prevBtn: VNode = h(
                      ...
                      [
                        h(
                          ...
                          [h('polyline', { attrs: { points: '15 18 9 12 15 6' } })],
                        ),
                      ],
                    );
                    const nextBtn: VNode = h(
                      ...
                      [
                        h(
                          ...
                          [h('polyline', { attrs: { points: '9 18 15 12 9 6' } })],
                        ),
                      ],
                    );
      ...
      // footer is independently opt-in.
