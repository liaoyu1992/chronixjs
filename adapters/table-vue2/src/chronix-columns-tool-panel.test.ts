...
describe('<ChronixColumnsToolPanel> (vue2)', () => {
  ...
  it('renders one checkbox per column with checked state reflecting `column.hide !== true`', () => {
    const wrapper = mount(ColumnsPanelForTest, {
      propsData: {
    ...
    expect((checkboxes.at(0).element as HTMLInputElement).checked).toBe(true);
    expect((checkboxes.at(1).element as HTMLInputElement).checked).toBe(true);
    expect((checkboxes.at(2).element as HTMLInputElement).checked).toBe(false); // hide: true
    expect((checkboxes.at(3).element as HTMLInputElement).checked).toBe(true);
    wrapper.destroy();
  });
  ...
  it('search input filters the visible checkbox list by column label substring (case-insensitive)', async () => {
    ...
    const search = wrapper.find('[data-testid="cx-columns-tool-panel-search"]');
