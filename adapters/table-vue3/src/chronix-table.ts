...
export interface TableHandle {
   ...
   * Always returns `0` when `showPagination` is `false`. When
  ...
  /**
   * open the column filter panel for `colId`.
   * Closes any other open filter panel first. No-op when
   * `showColumnFilterButton === false` or `colId` doesn't exist.
   */
  openColumnFilter(colId: string): void;
  /**
   * close the open column filter panel.
   * No-op when no panel is open.
   */
  closeColumnFilter(): void;
  /**
   * read the colId of the currently open
 ...
 * the consumer interacts with column filters. Consumers can
...
export const ChronixTable = defineComponent({
  ...
  props: {
     ...
     * `rowModelType: 'serverSide'` + `!showPagination`, the
    ...
    /**
     * when `true`, a funnel (filter)
     * icon button renders next to the ⋮ column-header menu button.
     * Clicking it opens a popover panel with a multi-condition
     * filter UI: each condition row has an operator dropdown +
     * value input; conditions combine via an AND/OR mode toggle.
     * The panel reads/writes a `MultiFilterSpec` entry for the
     * column through the existing `setFilter` / `getFilter` API.
     * Defaults to `false` (no funnel button rendered).
     */
  ...
  setup(props, ctx) {
    ...
    // header is a SIBLING of body, so the
    // body's `overflowX: auto` scroll does NOT propagate to
    // it. The SFC mirrors `bodyScrollLeft → headerEl.scrollLeft`
    // via an inline scroll listener so the
    // header cells stay column-aligned with body cells during
    // horizontal scroll. Header carries `overflowX: hidden`
    ...
    /**
     * Column filter panel state. Only one column-filter
     * panel open at a time; opening a new one auto-closes the
     * previous. The panel is hoisted to wrapper level (same as
     * the column-header menu) to escape the cell's
    ...
    /**
     * Read the current MultiFilterSpec for a column from the
     * internal filterSpec. Returns `null` when the column has no
     * multi-filter entry. Used by the filter panel to initialize
     * its editable condition rows.
     */
    ...
    /**
     * Write a MultiFilterSpec for a column into the internal
     * filterSpec, replacing any existing multi/text/number/set
     * entry for the same column. Pass `null` to remove the
     * column's filter entry entirely.
     */
    ...
    const serverSideRowsSynthesized = computed<readonly RowSpec[]>(() => {
      ...
      // Decision B.1: when showPagination,
    ...
    // !showPagination` — paginated mode keeps the existing Phase
    ...
    // for serverSide + showPagination mode. The client-side
    ...
    // subsequent click event (browser always fires click after
    // pointerup) can be suppressed in onClickCapture - otherwise
    // it reaches onBodyContentClick and triggers row selection.
    ...
    function onCellShiftClick(rowId: string, colId: string, e: MouseEvent): void {
      if (props.cellRangeSelection !== 'enabled') return;
      // A cell-range drag just ended - the browser fires a click
      // after pointerup. Suppress it so it doesn't trigger row
      // selection / cell-click via the delegated body handler.
    ...
    const handle: TableHandle = {
      ...
      getTotalPages(): number {
        // Decision B.1: serverSide+showPagination
    ...
    return () => {
       ...
       * suffixes. Used by header / body by composing with
       * the base class prefix (`cx-table-cell` / `cx-table-header-cell`).
       * Returns an empty array for center
      ...
      function buildRowDragGripCell(row: RowSpec, rowH: number): VNode {
        ...
        return h(
          ...
          isInactive
            ? []
            : [
                h('span', { class: 'cx-table-row-drag-cell__grip', 'aria-hidden': 'true' }, [
                  h(
                    'svg',
                    {
                      width: 10,
                    ...
                    [
                      h('circle', { cx: 2.5, cy: 3, r: 1.3 }),
                      h('circle', { cx: 7.5, cy: 3, r: 1.3 }),
                      h('circle', { cx: 2.5, cy: 8, r: 1.3 }),
                      h('circle', { cx: 7.5, cy: 8, r: 1.3 }),
                      h('circle', { cx: 2.5, cy: 13, r: 1.3 }),
                      h('circle', { cx: 7.5, cy: 13, r: 1.3 }),
                    ],
                  ),
                ]),
              ],
      ...
      const headerCellNodes: VNode[] = cells.map((cell) => {
        ...
        // Column filter funnel button. Renders when
        // props.showColumnFilterButton === true AND the column is
        // filterable AND it's not an actions column. Clicking opens
        // a popover panel with a multi-condition filter UI.
          ...
          ? (() => {
              ...
              const statusArea: VNode | null = props.showStatusBar
                ? (() => {
                    ...
                    const inner: VNode | string =
                      slot != null
                        ? (slot({ counts }) as unknown as VNode)
                        : defaultStatusBarText(counts);
                    return h(
                      'div',
                      {
              ...
              const cluster: VNode | null = props.showPagination
                ? (() => {
                    ...
                    // showPagination mode, the footer reads totals from
      ...
      // assemble wrapper children — header,
      // body, optional pagination footer.
      // footer is independently opt-in. The footer aggregate row now
      ...
      // Column filter panel overlay. Hoisted to wrapper level
      // (same as the column header menu) so it escapes the cell's
      // overflow:hidden. The panel reads/writes a MultiFilterSpec
      // for the open column via applyColumnMultiFilter.
        ...
        props.showColumnFilterButton === true && openFilterColId != null && filterPos != null
          ? (() => {
                ...
                return h('div', { class: 'cx-table-column-filter-row' }, [
                  ...
                  h(
                    ...
                    operators.map((op) =>
                      h(
                        'option',
                        { value: op.value, selected: row.operator === op.value },
                        op.label,
                      ),
                    ),
                  ),
              ...
              return h(
                ...
                {
                  ref: (el: unknown) => {
                    columnFilterPanelRef.value = el as HTMLElement | null;
                  },
                  class: 'cx-table-column-filter-panel',
                  'data-col-id': openFilterColId,
                  style: {
                    position: 'absolute',
                    left: `${pos.left}px`,
                    top: `${pos.top}px`,
